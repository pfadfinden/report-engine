...
/**
 * Loads a precompiled report (report.jasper) from a directory named after the
 * report from a base directory on the local filesystem, e.g.
 * {@code <reportsBaseDir>/<reportName>/report.jasper}.
 *
 * Resources referenced by the report at fill time (a {@code <template>}
 * pointing at a .jrtx style template, a custom font family's TTF files, an
 * image) are resolved first from that report's own directory, then - if not
 * found there - from a shared assets directory common to every report (style
 * template, fonts, logos live there by default; a report only needs its own
 * copy of something if it wants to override the shared default).
 */
public class LocalFilesystemReportLoader implements ReportLoader {
    ...
    public ReportDefinition load(String reportName) throws FailedToLoadReport {
        ...
        try {
            ...
            repositoryServices.add(new FileRepositoryService(jasperReportsContext, reportDir.getAbsolutePath(), false));
            if (sharedAssetsDir != null && sharedAssetsDir.isDirectory()) {
                repositoryServices
                        .add(new FileRepositoryService(jasperReportsContext, sharedAssetsDir.getAbsolutePath(), false));
            }
            ...
            if (existsInReportOrShared(reportDir, "fonts.xml")) {
                List<FontFamily> fontFamilies = SimpleFontExtensionHelper.getInstance()
