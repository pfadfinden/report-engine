...
/**
 * Coerces the raw, JSON-decoded parameter values coming from an HTTP request
 * body into the types a report's parameters actually declare
 * (JRParameter#getValueClass()). JasperReports does not coerce parameter
 * values itself - a Jackson-decoded Integer handed to a report expecting a
 * java.lang.Long, for example, fails fast inside the fill rather than being
 * silently widened.
 *
 * Shared by every HTTP-facing executor (local-report-executor,
 * azure-report-executor) so both apply the same coercion rules.
 *
 * Only the common scalar cases (numbers, strings, booleans) are handled;
 * anything else is passed through unchanged and left to fail fill-time with
 * a clear JasperReports error if it's genuinely incompatible.
 */
