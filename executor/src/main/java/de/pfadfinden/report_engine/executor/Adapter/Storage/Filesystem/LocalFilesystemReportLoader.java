...
public class LocalFilesystemReportLoader implements ReportLoader {
  ...
  public ReportDefinition load(String reportName) throws FailedToLoadReport {
    File reportDir = resolveReportDir(reportName);
  ...
  // reportName ultimately originates from an HTTP-facing reportId (see callers), so it must
  // never be allowed to escape reportsBaseDir via "..", an absolute path, or a symlink -
  // otherwise a caller could point the loader (and the JRLoader.loadObject deserialization
  // below) at an arbitrary file on the executor's filesystem.
  private File resolveReportDir(String reportName) throws FailedToLoadReport {
    File reportDir = new File(reportsBaseDir, reportName);
    try {
      File canonicalBase = reportsBaseDir.getCanonicalFile();
      File canonicalReportDir = reportDir.getCanonicalFile();
      if (!canonicalReportDir.toPath().startsWith(canonicalBase.toPath())) {
        throw new FailedToLoadReport("Invalid report id: " + reportName);
      }
      return reportDir;
    } catch (IOException e) {
      throw new FailedToLoadReport(e);
