...
/**
 * Verifies that a report referencing a .jrtx style template (via &lt;template&gt;) can be loaded
 * and filled when both files sit side by side under the reports base directory, i.e. that the
 * RepositoryService wired up by LocalFilesystemReportLoader actually resolves the template.
 *
 * <p>The report itself is built with the JRDesign* API rather than parsed from a hand-written
 * .jrxml: JasperReports 7's Jackson-based JRXML format is strict about exact element/attribute
 * names (e.g. no default XML namespace, "element" wrapper tags for band children, "bold" not
 * "isBold"), and getting that exactly right by hand isn't the point of this test - what's under
 * test is template *resolution* at fill time, not JRXML parsing. The .jrtx template itself,
 * referenced only by location (a plain file), is still real hand-written XML, so it does exercise
 * the loader.
 */
class LocalFilesystemReportLoaderTest {
  ...
  void loadsAndFillsReportWithJrtxTemplate(@TempDir Path reportsBaseDir) throws Exception {
    ...
    try (InputStream jrtx =
        getClass().getClassLoader().getResourceAsStream(reportName + "/TestStyles.jrtx")) {
    ...
    new FillReportService()
        .fill(reportDefinition, new HashMap<>(), conn, output, OutputFormat.XLSX);
