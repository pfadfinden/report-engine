...
/**
 * Signs and verifies short-lived download URLs for the local executor's own
 * /files endpoint, so that the /executions/{id}/download response always
 * carries a URL (never bytes/a redirect) - the same shape a cloud-hosted
 * executor would return via a real signed blob URL.
 */
