...
function toPrincipalRole(value: unknown): PrincipalRole | undefined {
  ...
  const { group_id, group_name, role, role_class, role_name, permissions } = raw;
...
export function toPrincipalRoles(value: unknown): ReadonlyArray<PrincipalRole> {
  ...
  return value.map(toPrincipalRole).filter((role): role is PrincipalRole => role !== undefined);
}
...
export function claimsToPrincipal(claims: IDToken, roles: ReadonlyArray<PrincipalRole>): Principal {
  return {
    id: claims.sub,
