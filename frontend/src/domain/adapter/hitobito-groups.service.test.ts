...
test('resolves the type for a group referenced by a role without subtree access', async () => {
  ...
  const sut = new HitobitoGroupsService(API_URL, API_TOKEN);
  const groups = await sut.findFor(principal([role({ groupId: '1', permissions: ['contact_data'] })]));

  expect(groups).toEqual([{ id: '1', name: 'Bundesamt', type: 'Group::Bundesamt', parentId: null }]);
...
test("expands a role's own layer group into its full subtree when it grants layer_and_below access", async () => {
  ...
  fetchMock.mockResolvedValueOnce(jsonResponse({ data: [groupResource('1', 'Bund', 'Group::Bund')] }));
  ...
  const sut = new HitobitoGroupsService(API_URL, API_TOKEN);
  const groups = await sut.findFor(principal([role({ groupId: '1', permissions: ['layer_and_below_full'] })]));
...
test("expands to the role's enclosing layer (not the role's own group) when the role is on a non-layer group", async () => {
  ...
  fetchMock.mockResolvedValueOnce(
    jsonResponse({
      data: [groupResource('779', 'Bundesamt', 'Group::Bundesgeschaeftsstelle', '1', '1')],
    }),
  );
  ...
  const sut = new HitobitoGroupsService(API_URL, API_TOKEN);
  const groups = await sut.findFor(principal([role({ groupId: '779', permissions: ['layer_and_below_full'] })]));
...
test('follows pagination links and deduplicates groups shared by multiple roles', async () => {
  ...
  fetchMock.mockResolvedValueOnce(jsonResponse({ data: [groupResource('2', 'Group Two', 'Group::Foo')] }));
...
test('throws when the Hitobito API responds with an error status', async () => {
  ...
  await expect(sut.findFor(principal([role({ groupId: '1' })]))).rejects.toThrow(/500/);
});
