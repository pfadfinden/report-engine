...
/**
 * Durable storage for filled report output, backed by the function app's own
 * storage account (AzureWebJobsStorage). The download endpoint hands back a
 * short-lived SAS URL to a blob here - never bytes, never a redirect - to
 * match the API contract shared with the local executor.
 */
public class BlobReportOutputStore {
    ...
    public void upload(String executionId, String extension, String contentType, InputStream content, long length) {
