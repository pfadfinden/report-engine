...
import de.pfadfinden.reports_engine.executor.Adapter.Storage.RemoteZip.RemoteZipReportLoader;
...
/**
 * Background worker: consumes a message enqueued by
 * TriggerReportExecutionFunction, fills the report against the reports
 * bundle fetched from mv_reports' released reports.zip (cached locally per
 * function instance), uploads the result to blob storage, and records the
 * final status - all durable, since Azure Functions are stateless/scale-out.
 */
public class ExecuteReportFunction {
    ...
    public void run(
        ...
        try {
            ...
            RemoteZipReportLoader loader = new RemoteZipReportLoader(reportsSourceUrl,
                    new File(System.getProperty("java.io.tmpdir"), "reports-cache"), REPORTS_CACHE_TTL);

            ReportDefinition reportDefinition = loader.load(task.reportId());
            Map<String, Object> parameters = ParameterCoercion.coerce(reportDefinition.report, task.parameter());

            ByteArrayOutputStream output = new ByteArrayOutputStream();
            try (Connection conn = DriverManager.getConnection(System.getenv("REPORT_SOURCEDATA_DATABASE_URL"))) {
                new FillReportService().fill(reportDefinition, parameters, conn, output, format);
            }

            byte[] resultBytes = output.toByteArray();
            outputStore.upload(task.executionId(), format.value(), format.contentType(),
                    new ByteArrayInputStream(resultBytes), resultBytes.length);
            statusStore.markDone(task.executionId());
        } catch (Exception e) {
            context.getLogger().severe("Failed to execute report " + task.reportId() + ": " + e.getMessage());
