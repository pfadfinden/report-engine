...
/**
 * Durable execution status, keyed by executionId, backed by the function
 * app's own storage account (AzureWebJobsStorage) - Azure Functions are
 * stateless/scale-out, so unlike the local executor's in-memory map this
 * has to be shared, durable state.
 *
 * upsertEntity(TableEntity) defaults to a merge, not a replace - markDone/
 * markFailed only touching status/errorMessage relies on that to leave the
 * outputFormat set by putPending() untouched for the rest of the record's
 * life.
 */
public class TableExecutionStatusStore {
    ...
    /** The format recorded at putPending time - needed at download time to pick the blob's extension/content-type. */
    ...
    public void markDone(String executionId) {
        tableClient.upsertEntity(
                new TableEntity(PARTITION_KEY, executionId).addProperty(STATUS_PROPERTY, ExecutionStatus.DONE.name()));
    }
