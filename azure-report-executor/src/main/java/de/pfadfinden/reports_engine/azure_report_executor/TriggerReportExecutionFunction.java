...
/**
 * HTTP entry point of the trigger/status/download API contract shared with
 * local-report-executor: POST /reports/{reportId}/executions. Records the
 * execution as PENDING and hands the actual fill off to the queue-triggered
 * ExecuteReportFunction worker, returning immediately.
 */
public class TriggerReportExecutionFunction {
    ...
    public HttpResponseMessage run(
        ...
        try {
            body = OBJECT_MAPPER.readValue(request.getBody().orElse(""), TriggerReportExecutionRequestBody.class);
        } catch (Exception e) {
        ...
        try {
            ...
            new TableExecutionStatusStore(storageConnectionString).putPending(body.executionId(), body.outputFormat());

            ReportExecutionMessage message = new ReportExecutionMessage(body.executionId(), reportId,
                    body.parameter(), body.outputFormat());
