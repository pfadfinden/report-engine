...
/**
 * Decorates a ReportFillWorker with a report.execution span, audit log, and usage metrics - kept
 * separate from DefaultReportFillWorker so that class stays purely about loading, coercing, and
 * filling a report, with observability layered on top instead of inlined into it.
 */
public class TracingReportFillWorker implements ReportFillWorker {

  private final ReportFillWorker delegate;

  public TracingReportFillWorker(ReportFillWorker delegate) {
  ...
  public long execute(
    ...
    try (Scope scope = span.makeCurrent()) {
      ...
      // Catches Throwable, not just Exception, so an Error still gets recorded here instead of
  ...
  private static Tracer tracer() {
    return GlobalOpenTelemetry.getTracer("de.pfadfinden.report_engine.azure_report_executor");
  }
}
