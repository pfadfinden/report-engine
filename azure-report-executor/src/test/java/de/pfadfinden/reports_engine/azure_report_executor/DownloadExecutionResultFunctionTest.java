...
class DownloadExecutionResultFunctionTest {
    ...
    void doneReturnsSignedDownloadUrl() throws Exception {
        ...
        try (MockedConstruction<TableExecutionStatusStore> ignoredStatusStore = mockConstruction(
                TableExecutionStatusStore.class,
                (mock, mockContext) -> {
            ...
            try (MockedConstruction<BlobReportOutputStore> ignoredOutputStore = mockConstruction(
                    BlobReportOutputStore.class,
                    (mock, mockContext) -> when(mock.generateDownloadUrl(eq("exec-1"), eq("pdf")))
