...
import de.pfadfinden.report_engine.preprocessor.Adapter.YamlMetadataReader;
...
@Command(
    name = "preprocess",
    commandListHeading = "%nTasks:%n",
    description =
        "%nPreprocesses report definitions from a source directory for subsequent deployment. Different processing tasks can be selected by providing them as arguments.%n",
    mixinStandardHelpOptions = true,
    subcommandsRepeatable = true,
    versionProvider = ManifestVersionProvider.class,
    synopsisSubcommandLabel = "[<TASKS>...]")
public class DefaultCommand implements Runnable {

  @Option(
      names = {"--source-dir"},
      defaultValue = "./reports",
      description = "Path to directory with report definitions (default: '${DEFAULT-VALUE}')",
  ...
  @Mixin protected TaskOptions options;
  ...
  public void readMetadata() {
    ...
    MetadataRepository metadataRepository =
        SqliteMetadataRepository.initNew(this.options.outputDir().getPath());
    MetadataIngestService preprocessor =
        new MetadataIngestService(metadataRepository, new YamlMetadataReader());
  ...
  /**
   * This method is executed if the preprocessor is called without any arguments. It calls default
